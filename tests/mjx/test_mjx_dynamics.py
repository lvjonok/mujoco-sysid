...
from mujoco_sysid.mjx import regressors as regx
...
def test_body_regressor():
    ...
    for _ in range(SAMPLES):
        ...
        mjxY = regx.body_regressor(spatial_v[:3], spatial_v[3:], spatial_dv[:3], spatial_dv[3:])

        assert np.allclose(mjY, mjxY, atol=1e-6)
