...
def parameters_map(parameters: jnp.ndarray, model: mjx.Model) -> mjx.Model:
    ...
    log_armature, log_damping, log_friction_loss = parameters[0], parameters[1], parameters[2]
    armature = jnp.exp(log_armature)
    damping = jnp.exp(log_damping)
    friction_loss = jnp.exp(log_friction_loss)
    return model.tree_replace(
        {
            "dof_armature": model.dof_armature.at[0].set(armature),
...
def load_data(file_path, skip_footer=0):
    data_array = np.genfromtxt(file_path, delimiter=",", skip_header=600, skip_footer=skip_footer)
    timespan = data_array[:, 0] - data_array[0, 0]
    sampling = np.mean(np.diff(timespan))
    angle = data_array[:, 1]
    velocity = data_array[:, 2]
    torque = data_array[:, 3]
    control = torque#data_array[:, 4]
    
    return timespan, sampling, angle, velocity, torque, control

# Initialize random key
key = jax.random.PRNGKey(0)

# Load the model
MJCF_PATH = "models/motor.xml"

model = mujoco.MjModel.from_xml_path(MJCF_PATH)
data = mujoco.MjData(model)
model.opt.integrator = IntegratorType.EULER

# # Set gains and biases for PD control
# kp = 50
# kd = 4
# model.actuator_gaintype[0] = 0  # Position servo
# model.actuator_biastype[0] = 1  # Affine transformation
# model.actuator_gainprm[0, 0] = kp
# model.actuator_biasprm[0, 1] = -kp  # Position feedback
# model.actuator_biasprm[0, 2] = -kd  # Velocity feedback
...
# Load learning data
LEARNING_DATA_PATH = "data/multiharmonic_1.csv"
timespan, sampling, angle, velocity, torque, control = load_data(LEARNING_DATA_PATH, skip_footer=50)
...
default_parameters = jnp.concatenate(
    [jnp.log(mjx_model.dof_armature), jnp.log(mjx_model.dof_damping), jnp.log(mjx_model.dof_frictionloss)]
)
...
mujoco.mj_saveLastXML("models/motor_estimated.xml", updated_mj_model)
...
TEST_DATA_PATH = "data/chirp_2.csv"
timespan, sampling, angle, velocity, torque, control = load_data(TEST_DATA_PATH, skip_footer=100)
